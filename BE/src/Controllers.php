...
$app->get('/pages/api/types', function() use($app) {
    $data = $app['dbs']['adminUser']->fetchALL("SELECT * FROM type");
...
$app->options('/pages/api/types', function() use($app) {
...
$app->post('/pages/api/types', function(Request $request) use($app) {
    $data = json_decode($request->getContent(), true);
    $request->request->replace(is_array($data) ? $data : array());
    $dbOut = $app['dbs']['adminUser']->fetchALL("SELECT type_name FROM type WHERE type_name = ?",array($data['type_name']));
    if ($dbOut == True) {
        return new Response('Current name already exists!',400);
    }
    else {
        $app['dbs']['adminUser']->insert('type', array(
            'type_name' => $data['type_name'],
            'type_template' => $data['template'],
            'type_add_text_fields' => $data['additionalTextField'],
            'type_add_img_fields' => $data['additionalImgField']
        ));
...
},'POST')->after(function (Request $request, Response $response) {
    $response->headers->set('Access-Control-Allow-Origin', '*');
    $response->headers->set('Access-Control-Allow-Methods', 'POST, GET, PUT, OPTIONS, PATCH, DELETE');
    $response->headers->set('Access-Control-Allow-Headers', 'X-CSRF-Token, X-Requested-With, Accept, Accept-Version, Content-Length, Content-MD5, Content-Type, Date, X-Api-Version, X-HTTP-Method-Override, Origin');
});
...
/////////////////////////////////////////////////////////////////////
$app->get('/pages/api/types/{id}', function($id) use($app) {
...
$app->options('/pages/api/types/{id}', function($id) use($app) {
    return new Response('',200);
})->after(function (Request $request, Response $response) {
    $response->headers->set('Access-Control-Allow-Origin', '*');
    $response->headers->set('Access-Control-Allow-Methods', 'POST, GET, PUT, OPTIONS, PATCH, DELETE');
...
$app->put('/pages/api/types/{id}', function($id,Request $request) use($app) {
    $data = json_decode($request->getContent(), true);
    $request->request->replace(is_array($data) ? $data : array());
    ...
    $app['dbs']['adminUser']->update('type', $dbInput, array('type_id' =>intval($id)));
...
$app->delete('/pages/api/types/{id}', function($id) use($app) {
